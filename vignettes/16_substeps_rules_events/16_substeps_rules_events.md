...
3.  **Rules** (`AbstractTransitionRule`) emit signed `StageFlux` /
    `StageTransfer` effects; **events** (`AbstractScheduledEvent`)
    mutate the system on specific days.
...
```
    ...
    PopulationSystem(1 components: pop, 2 state vars)
...
```
    ...
    typeof(sub_resource).name.name = StateUpdateSubstep
    true
...
```
    ...
    typeof(sub_log).name.name = CustomSubstep
...
```
    ...
    typeof(flx).name.name = StageFlux
    typeof(tfr).name.name = StageTransfer
    typeof(eff).name.name = RuleEffect

## Transition rules

All four concrete rule types are `<: AbstractTransitionRule` and have
the shared `apply_rule(rule, sys, day, p)` interface.

``` julia
println("apply_rule method count = ", length(methods(apply_rule)))
```

    apply_rule method count = 4
...
```
    ...
    typeof(rr).name.name = ReproductionRule
...
```
    ...
    typeof(mr).name.name = MortalityRule
...
```
    ...
    typeof(cr).name.name = CustomRule
...
```
    ...
    typeof(pulse).name.name = PulseRelease
    true
...
```
    ...
    typeof(sdr).name.name = SingleDayRelease
...
```
    ...
    typeof(ce).name.name = CustomEvent
...
```
    ...
    retcode = Success
    days    = [0, 1, 2, 3, 4, 5]
    N       = [15.0, 33.625, 59.876, 109.135, 270.483, 323.792]
    R       = [100.0, 99.0, 98.0, 97.0, 96.0, 95.0]
    triggered events    = 6
    rule-log keys       = [:background, :graduate, :noop, :spawn]

## Summary

- Substeps: `AbstractDailySubstep`, `apply_substep!`,
  `StateUpdateSubstep`, `CustomSubstep`.
